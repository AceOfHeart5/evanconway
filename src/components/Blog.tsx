...
const BlogList = () => {
    ...
    if (BLOG.length === 0) return <Typography>No posts yet. Check back soon!</Typography>;

    return <Stack>
        { BLOG.map(post => <Card>
            <CardActionArea onClick={() => setSearchParams({ postid: String(post.date.getTime()) })}>
                <CardContent>
...
const Post = ({ postId }: PostProps) => {
    ...
    if (post === undefined) return <Typography>{'Blog post not found :('}</Typography>

    return <>
        <Button onClick={() => navigate("blog", { replace: true })}>{"< Back"}</Button>
